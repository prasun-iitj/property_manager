...
class LendingDetailScreen extends StatelessWidget {
  ...
  Future<void> exportPdf(BuildContext context) async {
    ...
    var loanDoc = await loanRef.get();
    var loanData = loanDoc.data() as Map<String, dynamic>;

    var installmentsSnap =
        await loanRef.collection('installments').orderBy('date').get();
    ...
    await PdfService.generateLedgerPdf(
      loanData['name'],
      loanData['principal'].toDouble(),
      loanData['interestRate'].toDouble(),
      installments,
    );
  }

  // 🔥 RECALCULATION
  Future<void> _recalculateLoan(DocumentReference loanRef) async {
    var snap = await loanRef.collection('installments').get();
  ...
  Future<void> deleteInstallment(
    ...
    await _recalculateLoan(loanRef);
